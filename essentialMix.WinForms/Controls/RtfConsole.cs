...
public class RtfConsole : RichTextBox
{
	private const string CAT_MODE = "Mode";
	private const string CAT_BEHAVIOUR = "Behaviour";
	private const string CAT_APPEARANCE = "Appearance";
	...
	public Color ErrorColor { get; set; } = Color.Crimson;
	...
	public bool WriteInput(string value, Color color)
	{
		if (!WriteToProcess(value)) return false;
		this.InvokeIf(() =>
		{
			_writer.FlushBuffer();
			this.AppendText(value, color);
		});
		return true;
	}
	...
	public void WriteLine(string value, Color color)
	{
		...
		{
			_writer.FlushBuffer();
			this.AppendText(value + _writer.NewLine, color);
			_inputStart = SelectionStart;
		});
	}

	public void WriteError(string value) { Write(value, ErrorColor); }
	public void WriteErrorLine(string value) { WriteLine(value, ErrorColor); }
