...
using essentialMix.WinForms.Design;
...
public class Console : TextBox
{
	...
	[Category(CAT_BEHAVIOUR)]
	[Description("Characters read by the ReadFile or ReadConsole function are written to the active screen buffer as they are read. This mode can be used only if the LineInput mode is also enabled.")]
	public bool EchoInput { get; set; } = true;

	[Category(CAT_BEHAVIOUR)]
	[Description("The ReadFile or ReadConsole function returns only when a carriage return character is read. If this mode is disabled, the functions return when one or more characters are available.")]
	public bool LineInput { get; set; } = true;

	[Category(CAT_BEHAVIOUR)]
	[Description("When enabled, text entered in a console window will be inserted at the current cursor location and all text following that location will not be overwritten. When disabled, all following text will be overwritten.")]
	public bool InsertMode { get; set; } = true;

	[Category(CAT_BEHAVIOUR)]
	[Description("If the mouse pointer is within the borders of the console window and the window has the keyboard focus, mouse events generated by mouse movement and button presses are placed in the input buffer. These events are discarded by ReadFile or ReadConsole, even when this mode is enabled.")]
	public bool MouseInput { get; set; } = true;

	[Category(CAT_BEHAVIOUR)]
	[Description("User interactions that change the size of the console screen buffer are reported in the console's input buffer. Information about these events can be read from the input buffer by applications using the ReadConsoleInput function, but not by those using ReadFile or ReadConsole.")]
	public bool WindowInput { get; set; }

	[Category(CAT_BEHAVIOUR)]
	[Description("CTRL+C is processed by the system and is not placed in the input buffer. If the input buffer is being read by ReadFile or ReadConsole, other control keys are processed by the system and are not returned in the ReadFile or ReadConsole buffer. If the LineInput mode is also enabled, backspace, carriage return, and line feed characters are handled by the global::System.")]
